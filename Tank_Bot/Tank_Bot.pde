...
History
 	001 - Initial release  
        ...
        003 - Adding Basic GP2D12 read and first Set Moves using compass
                ...
                HSxxx#        Set Heading to xxx (0-359) Degeres using a right spin
                SMyyxxx#   Set Move commands
                  SRxxx#   Spin Right by xxx degeres (0-359)
                  SLxxx#   Spin Left by xxx degeres  (0-359)
        004 - Adding Pan/Tilt Servos
                Pan servo on pin 9, Tilt on pin 10
                Controled by commands or Zbutton and Accelerometer on Chuck
                New Commands
                PRxxxx#    Pan-Roll in xxxx degeres ()
                TPxxxx#    Tilt-Pitch in xxxx degeres ()
...
#define PAN_OFFSET 5
#define TILT_OFFSET -48
int Roll = 0;         // Holds last unadjusted Roll from Chuck
int Pitch = 0;        // Holds last unadjusted Pitch from Chuck
...
void setup(){
  ...
  // Attach Servo pins and center
   pan.attach(9);  // attaches the pan servo on pin 9
   pan.write(90 + PAN_OFFSET);  // attaches the pan servo on pin 9
   tilt.attach(10);  // attaches the tilt servo on pin 10
   tilt.write(90 + TILT_OFFSET);  // attaches the tilt servo on pin 10
 ...
 ****************************************************/
 ...
 /**************************************************** 
 *  Check chuck for z button and gen pan/tilt commands
 *  
 ****************************************************/
  chuck.update();
  if (chuck.buttonZ) {  //hold button Z to send pan/tilt commands,
      Roll =  chuck.readRoll();
      Pitch = chuck.readPitch();
      UpdatePanTilt(&Roll , &Pitch );
  } // end if (chuck.buttonZ)
...
/**************************************************** 
 *  Update PanTilt servos posotions 
 *  Takes Pitch and Roll as generated by WiiChuckClass.h
 *
 ****************************************************/
void UpdatePanTilt(int* curRoll, int* curPitch) {

  pan.write(90 + *curRoll + PAN_OFFSET);
  tilt.write(*curPitch + TILT_OFFSET);

} // end void UpdatePanTilt(int curRoll, int curPitch )

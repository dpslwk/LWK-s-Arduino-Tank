...
History
 	001 - Initial release  
        ...
        004 - Adding Pan/Tilt
               Holding Zbutton and Accelerometer on Chuck sends new Roll Pitch readings via MIRF
               New Commands
 ...
 ****************************************************/
  ...
  if (chuck.buttonZ) {  //hold button Z to send pan/tilt commands, 
    UpdatePanTilt(chuck.readRoll(), chuck.readPitch() );
...
/**************************************************** 
 *  Generate PanTilt command and send via MIRF
...
void UpdatePanTilt(int curRoll, int curPitch) {
  ...
  // construct data packet
  // packet = 'PR' + curRoll + "#TP" + curPitch + '#';
  
  sprintf(packet, "PR%d#TP%d#", curRoll, curPitch);

/* 
  Serial.print("Chuck Roll,Pitch:\t");
